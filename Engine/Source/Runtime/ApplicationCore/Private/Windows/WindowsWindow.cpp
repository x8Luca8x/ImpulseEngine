...
LRESULT FWindowsWindow::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		if (window && window->bIsMovingWindow)
		{
			// Get delta cursor pos
			...
			int32 NewX = CursorPos.x - window->WindowDefinition->WidthDesiredSize / 2;
			int32 NewY = CursorPos.y - window->WindowDefinition->HeightDesiredSize / 2;
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (window)
		{
			...
			if (CursorPos.x >= WindowRect.left && CursorPos.x <= WindowRect.right && CursorPos.y >= WindowRect.top && CursorPos.y <= WindowRect.bottom)
			{
				// Set window as moving

				window->bIsMovingWindow = true;
			}
		}
	...
	{
		if (window && window->bIsMovingWindow)
			// Set window as not moving
			window->bIsMovingWindow = false;
